...
public:
  ...
  static uint8_t encodeMessage(const LoRaTxMessage& msg, uint8_t* buffer, uint8_t maxLen);
  
  // Decode message from bytes
  static bool decodeMessage(const uint8_t* buffer, uint8_t len, LoRaRxMessage& msg);
  
  // Helper to set default header
  static void setDefaultHeader(LoRaHeader& header, uint8_t dst, uint8_t src, uint8_t id = 0,
